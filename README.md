# Hi there 👋 I'm Sarwar Olumee

### Software Engineering Student | Backend Developer | Laravel Enthusiast

I'm a passionate Software Engineering student from Afghanistan with a strong interest in building scalable, maintainable, and user-friendly web applications. I enjoy solving real-world problems through software and continuously improving my development skills by learning modern technologies and best practices.

---

## 👨‍💻 About Me

| Information           | Details                                       |
| --------------------- | --------------------------------------------- |
| 🎓 Education          | BSc in Software Engineering                   |
| 🏫 University         | Herat University                              |
| 📍 Location           | Herat, Afghanistan                            |
| 💼 Focus              | Backend Web Development                       |
| ❤️ Favorite Framework | Laravel                                       |
| 🌱 Currently Learning | Livewire, Design Patterns, Clean Architecture |
| 🎯 Goal               | Become a Professional Laravel Developer       |

---

## 🚀 Tech Stack

### Backend

* PHP
* Laravel
* Livewire
* REST APIs

### Frontend

* HTML5
* CSS3
* JavaScript
* Bootstrap
* Tailwind CSS

### Database

* MySQL

### Tools

* Git
* GitHub
* Composer
* NPM
* Vite
* VS Code
* XAMPP

---

## 📚 Currently Learning

* Advanced Laravel
* Livewire
* Design Patterns
* SOLID Principles
* Software Architecture
* REST API Development

---

## 💼 Featured Projects

| Project                        | Description                                                                                              |
| ------------------------------ | -------------------------------------------------------------------------------------------------------- |
| 📦 Inventory Management System | Complete inventory system with sales, purchases, warehouse management, POS, reports, and stock tracking. |
| 🏠 Dormitory Management System | Resident management, room allocation, contracts, billing, and maintenance request management.            |
| 🌐 Personal Portfolio          | Responsive portfolio website showcasing projects and skills.                                             |

---

## 🎯 Goals for 2026

* ✅ Master Laravel
* ✅ Master Livewire
* ✅ Learn Docker
* ✅ Learn Testing in Laravel
* ✅ Contribute to Open Source
* ✅ Build Production-Ready Applications

---

## 💡 Core Skills

* Clean Code
* MVC Architecture
* Object-Oriented Programming
* RESTful API Development
* Database Design
* Authentication & Authorization
* Git Workflow
* Problem Solving

---

## 🌍 Languages

| Language | Level        |
| -------- | ------------ |
| Dari     | Native       |
| English  | Intermediate |
|          |              |

---

## 📫 Contact

* GitHub: https://github.com/sarwarOlumee21
* Email:[sarwarolumi220@gmail.com](mailto:sarwarolumi220@gmail.com)
* +93793448759

---

> **"Code with purpose. Learn continuously. Build solutions that make a difference."**
